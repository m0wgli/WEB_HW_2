...
class Notebook(UserDict):
    ...
    def search(self, param):
        ...
        for key, value in self.items():
            if param.lower() in str(key).lower() or param.lower() in str(value).lower():
                result += f"{key} {value}\n"
        if not result:
    ...
    def delete_note(self, param):
        keys_to_delete = [key for key in self.keys() if param.lower() in key.lower()]
    ...
    def sorting_by_keyword(self):
        sorted_data = dict(
            sorted(self.data.items(), key=lambda x: str(x[1].keywords).lower())
        )
