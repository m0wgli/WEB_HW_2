...
def exchange_rates():
    bank_ua_responce = requests.get(
        "https://bank.gov.ua/NBUStatService/v1/statdirectory/exchange?json"
    )
    privat_response = requests.get(
        "https://api.privatbank.ua/p24api/pubinfo?exchange&coursid=5"
    )
    data_bank, data_privat = bank_ua_responce.json(), privat_response.json()
    usd_privat_buy, usd_privat_sale = float(data_privat[1]["buy"]), float(
        data_privat[1]["sale"]
    )
    eur_privat_buy, eur_privat_sale = float(data_privat[0]["buy"]), float(
        data_privat[0]["sale"]
    )
